...
class PCAAnalysis:
    ...
    def plot_component_loadings(self, n_components: int = 3) -> plt.Figure:
        ...
        for i, (pc_name, loadings) in enumerate(components_df.iterrows()):
            ...
            ax.set_title(
                f"{pc_name} Loadings (Explained Variance: {self._explained_variance_ratio[i]:.3f})"
            )
    ...
    def analyze_factor_relationship(
        self, factors_df: pd.DataFrame, n_components: int = 3
    ) -> pd.DataFrame:
        if not self.is_fitted:
        ...
        correlations = pd.DataFrame(
            index=pc_aligned.columns, columns=factors_aligned.columns
        )

        for pc in pc_aligned.columns:
            for factor in factors_aligned.columns:
                correlations.loc[pc, factor] = pc_aligned[pc].corr(
                    factors_aligned[factor]
                )

        return correlations.astype(float)

    def plot_factor_correlations(
        self, correlations: pd.DataFrame, title: str = "PC-Factor Correlations"
    ) -> plt.Figure:
